...
update_world_server() {
    ...
    for attempt in 1 2 3; do
        if MYSQL_PWD="$STARLOCO_DB_PASSWORD" mariadb --skip-ssl -h "${MARIADB_HOST:-mariadb}" \
    ...
    VERIFY_KEY=$(MYSQL_PWD="$STARLOCO_DB_PASSWORD" mariadb --skip-ssl -h "${MARIADB_HOST:-mariadb}" \
        -u "${STARLOCO_DB_USER:-starloco}" \
        starloco_login \
        -sN -e "SELECT \`key\` FROM world_servers WHERE id=${GAME_SERVER_ID:-601};")

    if [ "${VERIFY_KEY}" = "${SAFE_GAME_SERVER_KEY}" ]; then
        echo "World server key verified"
    else
        echo "WARNING: World server key mismatch"
    fi
}
...
system.server.login.version ${GAME_SERVER_VERSION:-1.41.9}
...
system.server.game.version ${GAME_SERVER_VERSION:-1.41.9}
EOF
