...
public class LoginServer {
    ...
    private final Map<String, LoginClient> clients = new TreeMap<>();
    ...
    /**
     * Publishes a session only after its password has been verified. Replacing
     * the map entry before closing the previous client makes its conditional
     * unregister harmless and prevents a name-only login attempt from evicting
     * an authenticated session.
     */
    public void registerAuthenticated(LoginClient client) {
        if (client == null || client.getAccount() == null) {
            throw new IllegalArgumentException("An authenticated account is required");
        }

        LoginClient previous;
        synchronized (clients) {
            previous = clients.put(client.getAccount().getName(), client);
        }
    ...
    public void unregisterAuthenticated(LoginClient client) {
        if (client == null || client.getAccount() == null) {
            return;
        }
        synchronized (clients) {
            clients.remove(client.getAccount().getName(), client);
        }
    }

    public LoginClient getAuthenticatedClient(String accountName) {
        synchronized (clients) {
            return clients.get(accountName);
        }
    }
