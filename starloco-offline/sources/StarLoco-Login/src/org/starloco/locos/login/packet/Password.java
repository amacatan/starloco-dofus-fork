...
public class Password {
    ...
    static String decryptPassword(String pass, String key) {
        if (pass == null || key == null || key.isEmpty()) {
            return null;
        }
        ...
        if (pass.isEmpty() || pass.length() % 2 != 0 || pass.length() / 2 > key.length()) {
            return null;
        }
    ...
    static boolean isValidPass(String password, String passHash) {
        return password != null && passHash != null && encrypt(password).equals(passHash);
    }
}
