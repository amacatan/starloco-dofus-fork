...
public class GameClient {
    ...
    public static String packetForLog(String packet) {
        if (packet == null) {
            return "[NULL]";
        }
        if (packet.length() >= 2 && packet.regionMatches(true, 0, "AT", 0, 2)) {
            return "AT[REDACTED]";
        }
        return packet;
    }
    ...
    public void parsePacket(String packet) throws InterruptedException {
        ...
        if(Logging.USE_LOG) {
            String packetForLog = packetForLog(packet);
            if (this.player != null) {
    ...
    private void switchCharacter() {
        // Character switch introduced in 1.39.8 and still used by Retro 1.41.9.
