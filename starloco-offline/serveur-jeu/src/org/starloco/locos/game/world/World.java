...
//        Minotoror.initialize();
    ...
    public void loadExtraMonster() {
        for (Entry<Integer, Map<String, Map<String, Integer>>> monster : extraMonstre.entrySet()) {
            int monsterId = monster.getKey();
            try {
                Set<GameMap> possibleMaps = new LinkedHashSet<>();
                Map<String, Map<String, Integer>> placements = monster.getValue();

                if (placements == null || placements.isEmpty()) {
                    logger.warn("Skipping extra-monster {}: no placement was configured.", monsterId);
                    continue;
                }

                for (Entry<String, Map<String, Integer>> placement : placements.entrySet()) {
                    Map<String, Integer> subAreaChances = placement.getValue();
                    if (subAreaChances == null || subAreaChances.isEmpty()) {
                        possibleMaps.addAll(findEligibleExtraMonsterMaps(placement.getKey(), null));
                        continue;
                    }

                    for (Entry<String, Integer> subAreaChance : subAreaChances.entrySet()) {
                        List<GameMap> placementMaps = findEligibleExtraMonsterMaps(
                                placement.getKey(),
                                subAreaChance.getKey()
                        );
                        Integer chance = subAreaChance.getValue();
                        if (chance != null && chance != -1) {
                            for (GameMap map : placementMaps) {
                                map.addMobExtra(monsterId, chance);
                            }
                        }
                        possibleMaps.addAll(placementMaps);
                    }
                }

                if (possibleMaps.isEmpty()) {
                    logger.warn("Skipping extra-monster {}: no eligible map was found.", monsterId);
                    continue;
                }

                if (getMonstre(monsterId) == null) {
                    logger.warn("Skipping extra-monster {}: its monster template does not exist.", monsterId);
                    continue;
                }

                List<GameMap> mapsForMonster = new ArrayList<>(possibleMaps);
                GameMap randomMap = mapsForMonster.size() == 1
                        ? mapsForMonster.get(0)
                        : mapsForMonster.get(Formulas.getRandomValue(0, mapsForMonster.size() - 1));
                if (randomMap.loadExtraMonsterOnMap(monsterId)) {
                    extraMonstreOnMap.put(monsterId, randomMap);
                } else {
                    logger.warn("Skipping extra-monster {}: no valid monster group could be created.", monsterId);
                }
            } catch (RuntimeException e) {
                logger.warn("Skipping extra-monster {} after invalid placement data: {}", monsterId, e.getMessage());
            }
        }
    }

    List<GameMap> findEligibleExtraMonsterMaps(String areaIds, String subAreaIds) {
        Set<GameMap> eligibleMaps = new LinkedHashSet<>();

        for (int areaId : parseExtraMonsterAreaIds(areaIds)) {
            Area area = areas.get(areaId);
            if (area == null) {
                logger.debug("Ignoring unknown area {} for an extra-monster.", areaId);
                continue;
            }
            addEligibleExtraMonsterMaps(eligibleMaps, area.getMaps());
        }

        for (int subAreaId : parseExtraMonsterAreaIds(subAreaIds)) {
            SubArea subArea = subAreas.get(subAreaId);
            if (subArea == null) {
                logger.debug("Ignoring unknown sub-area {} for an extra-monster.", subAreaId);
                continue;
            }
            addEligibleExtraMonsterMaps(eligibleMaps, subArea.getMaps());
        }

        return new ArrayList<>(eligibleMaps);
    }

    static List<Integer> parseExtraMonsterAreaIds(String areaIds) {
        if (areaIds == null || areaIds.trim().isEmpty()) {
            return Collections.emptyList();
        }

        Set<Integer> parsedIds = new LinkedHashSet<>();
        for (String token : areaIds.split(",", -1)) {
            String trimmedToken = token.trim();
            if (trimmedToken.isEmpty()) {
                continue;
            }
            try {
                int id = Integer.parseInt(trimmedToken);
                if (id >= 0) {
                    parsedIds.add(id);
                }
            } catch (NumberFormatException ignored) {
                // Invalid placement tokens are optional data and must not abort world loading.
            }
        }
        return new ArrayList<>(parsedIds);
    }

    private static void addEligibleExtraMonsterMaps(Set<GameMap> eligibleMaps, Collection<GameMap> candidates) {
        if (candidates == null) {
            return;
        }
        for (GameMap map : candidates) {
            if (map != null && !map.haveMobFix() && map.isPossibleToPutMonster()) {
                eligibleMaps.add(map);
            }
        }
    }
